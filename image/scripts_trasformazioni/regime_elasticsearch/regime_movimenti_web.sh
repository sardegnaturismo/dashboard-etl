...
if [ $CONNECTION -eq 4 ]
 then echo -e "Indice : movimenti \n Job : JOB_REGIME_MOVIMENTI_MOV \n Tentativi connessione esauriti" | mail -s " DASHBOARD OPERATORE - Aggiornamento fallito " $DESTINATARI
	...
	while [ $OK -lt 2 ]; do
		...
		nohup /opt/pentaho/data-integration/kitchen.sh /rep:"${POSTGRES_REPO}" /job:"J_QUAD_MONGO_ELASTIC_WEB_OPE" /dir:/SIRED/QUERY_CHECK/WEB_MONGO_ELASTIC_OPE /user:admin /pass:admin /level:Basic&>> ${LOGFILE_QUAD}.log
