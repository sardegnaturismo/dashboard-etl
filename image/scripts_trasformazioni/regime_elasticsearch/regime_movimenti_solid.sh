...
INDEX="movimenti_solid"
TYPE="mov_solid"
LOGFILE_JOB="/opt/pentaho/log_caricamenti/regime_mov_solid"
LOGFILE_QUAD="/opt/pentaho/log_caricamenti/quad_mov_solid"
DATA=$(date +%Y/%m/%d)

echo "INIZIO MOV SOLID $(date)"
...
while [ $CONNECTION -lt 4 ] && [ $RETRY -lt 2 ]; do
	...
	nohup /opt/pentaho/data-integration/kitchen.sh /rep:"${POSTGRES_REPO}" /job:"JOB_REGIME_MOVIMENTI_SOLID" /dir:/SIRED/REGIME/MOVIMENTI_SOLID /user:admin /pass:admin /level:Basic &> ${LOGFILE_JOB}.log	
		...
		if [ $? -eq 0 ]
	 	 then let "CONNECTION=$CONNECTION + 1"	
			...
			#analizzo file di log 
                	grep "${DATA}.*JOB_REGIME_MOVIMENTI_SOLID\ -\ Terminata\ jobentry.*\(risultato=\[false\]\)" ${LOGFILE_JOB}.log
        		if [ $? -ne 0 ]
         	 	 then break
...
if [ $CONNECTION -eq 4 ]
 then echo -e "Indice : movimenti_solid \n Job : JOB_REGIME_MOVIMENTI_SOLID \n Tentativi connessione esauriti" | mail -s " DASHBOARD CITTADINO - Aggiornamento fallito " $DESTINATARI
	...
	while [ $OK -lt 2 ]; do
		...
		nohup /opt/pentaho/data-integration/kitchen.sh /rep:"${POSTGRES_REPO}" /job:"J_QUAD_MONGO_ELASTIC_SOLID" /dir:/SIRED/QUERY_CHECK/SOLID_MONGO_ELASTIC /user:admin /pass:admin /level:Basic &>> ${LOGFILE_QUAD}.log
